...
$barber = QueryExecutor::getInstance()->getBarber("{$_GET["barberId"]}");
...
<head>
    <meta charset="UTF-8">
    <title>Barbershop - Изменение данных о барбере</title>
    ...
    <div class="content">
        <?php if(Access::isAdministrator()): ?>
            <div class="header-block">
                <h1>Изменение данных о барбере</h1>
            </div>
            <div class="form-block">
                <form action=".?barberId=<?php echo $_GET["barberId"]; ?>&photo=<?php echo $barber["photo"]; ?>" method="post" enctype="multipart/form-data">
                    <div class="form-block-image-block">
                        <div class="form-block-image-block-container">
                            <img id="barber-photo" name="photo" src="<?php echo "http://{$_SERVER["SERVER_NAME"]}/Resources/Images/Upload/{$barber["photo"]}"; ?>">
                        </div>
                    </div>
                        ...
                        <div class="form-block-row">
                            ...
                            <div id="form-block-row-column-input" class="form-block-row-column">
                                <div class="form-block-row-column-input-text">
                                    <input type="text" name="fullName" value="<?php echo isset($_SESSION["params"]["editBarber"]["fullName"]) ? $_SESSION["params"]["editBarber"]["fullName"] : $barber["full_name"]; ?>">
                                </div>
                            </div>
                        </div>
                        <div class="form-block-row">
                                ...
                                <div class="form-block-row-column-input-select">
                                    <select name="experienceId">
                                        <?php foreach ($experiences as $experience): ?>
                                            <option value="<?php echo $experience["id"]; ?>" <?php echo isset($_SESSION["params"]["editBarber"]["experienceId"]) ? ($experience["id"] == $_SESSION["params"]["editBarber"]["experienceId"] ? 'selected="selected"' : "") : ($experience["id"] == $barber["experience_id"] ? 'selected="selected"' : ""); ?>><?php echo $experience["name"]; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-block-row-column-input-select">
                                    <select name="levelId">
                                        <?php foreach ($levels as $level): ?>
                                            <option value="<?php echo $level["id"]; ?>" <?php echo isset($_SESSION["params"]["addBarber"]["levelId"]) ? ($level["id"] == $_SESSION["params"]["addBarber"]["levelId"] ? 'selected="selected"' : "") : ($level["id"] == $barber["level_id"] ? 'selected="selected"' : ""); ?>><?php echo $level["name"]; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>
                            </div>
